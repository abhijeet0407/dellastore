...
if ( !class_exists( 'Radium_Theme_Importer' ) ) {
  ...
  class Radium_Theme_Importer {
    ...
    public function set_demo_theme_options( $file ) {
      ...
      if ( ! file_exists( $file ) ) {
        wp_die(
          __( 'Theme options Import file could not be found. Please try again.', 'arw-leka' ),
          '',
      ...
      $data = arexworks_wp_filesystem()->get_contents( $file );
    ...
    function process_widget_import_file( $file ) {
      ...
      if ( ! file_exists( $file ) ) {
        wp_die(
          __( 'Widget Import file could not be found. Please try again.', 'arw-leka' ),
          '',
          array( 'back_link' => true )
        );
      }
    ...
    public function import_widgets( $data ) {
      ...
      foreach ( $data as $sidebar_id => $widgets ) {
        ...
        if ( isset( $wp_registered_sidebars[$sidebar_id] ) ) {
          ...
          $sidebar_message = __( 'Sidebar does not exist in theme (using Inactive)', 'arw-leka' );
        }
        ...
        foreach ( $widgets as $widget_instance_id => $widget ) {
          ...
          if ( ! $fail && ! isset( $available_widgets[$id_base] ) ) {
            ...
            $widget_message = __( 'Site does not support widget', 'arw-leka' ); // explain why widget not imported
          }
          ...
          if ( ! $fail && isset( $widget_instances[$id_base] ) ) {
            ...
            foreach ( $single_widget_instances as $check_id => $check_widget ) {
              ...
              if ( in_array( "$id_base-$check_id", $sidebar_widgets ) && (array) $widget == $check_widget ) {
                ...
                $widget_message = __( 'Widget already exists', 'arw-leka' ); // explain why widget not imported
          ...
          if ( ! $fail ) {
            ...
            if ( $sidebar_available ) {
              ...
              $widget_message = __( 'Imported', 'arw-leka' );
            } else {
              $widget_message_type = 'warning';
              $widget_message = __( 'Imported to Inactive', 'arw-leka' );
            }
          ...
          $results[$sidebar_id]['widgets'][$widget_instance_id]['title'] = $widget->title ? $widget->title : __( 'No Title', 'arw-leka' ); // show "No Title" if widget instance is untitled

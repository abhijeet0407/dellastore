...
if ( !class_exists( 'ReduxFramework_wbc_importer' ) ) {
    ...
    class ReduxFramework_wbc_importer {
        ...
        public function render() {
            ...
            echo '</fieldset></td></tr><tr><td colspan="2"><fieldset class="wbc_importer">';
            ...
            if ( !empty( $this->field['wbc_demo_imports'] ) ) {
                ...
                foreach ( $this->field['wbc_demo_imports'] as $section => $imports ) {
                    ...
                    if ( !array_key_exists( 'imported', $imports ) ) {
                        ...
                        $imported = false;
                        $import_message = esc_html__( 'Import Demo', 'arw-leka' );
                    }else {
                        $imported = true;
                        $extra_class = 'active imported';
                        $import_message = esc_html__( 'Demo Imported', 'arw-leka' );
                    }
                    $not_import_content = false;
                    if ( array_key_exists( 'not_import_content', $imports ) ) {
                        $not_import_content = true;
                    }
                    echo '<div class="wrap-importer theme '.$extra_class.'" data-demo-id="'.esc_attr( $section ).'"  data-nonce="' . $nonce . '" id="' . $this->field['id'] . esc_attr( $section ) . '-custom_imports">';
                    ...
                    if ( false == $imported ) {
                        echo '<div class="wbc-importer-buttons"><span class="spinner">'.esc_html__( 'Please Wait...', 'arw-leka' ).'</span><span class="button-primary importer-button import-demo-data">' . __( 'Import Demo', 'arw-leka' ) . '</span></div>';
                    }else {
                        echo '<div class="wbc-importer-buttons button-secondary importer-button">'.esc_html__( 'Imported', 'arw-leka' ).'</div>';
                        echo '<span class="spinner">'.esc_html__( 'Please Wait...', 'arw-leka' ).'</span>';
                        echo '<div id="wbc-importer-reimport" class="wbc-importer-buttons button-primary import-demo-data importer-button">'.esc_html__( 'Re-Import', 'arw-leka' ).'</div>';
                    }
                    echo '</div>';
                    echo '<div class="theme-extra-options">';
                        echo '<label for="'.$this->field['id'] . esc_attr( $section ).'"><input id="'.$this->field['id'] . esc_attr( $section ).'" type="checkbox" '.($not_import_content ? 'checked="checked"' : '').' class="checkbox extra-import-options"/> Without Content</label>';
                    echo '</div>';
                    echo '</div>';
                }

            } else {
                echo "<h5>".esc_html__( 'No Demo Data Provided', 'arw-leka' )."</h5>";
            }
